...
async function updateUserInfoFromSupabase() {
    try {
        ...
        // Verificar se está na página de paciente (index.html)
        const currentPage = window.location.pathname.split('/').pop();
        if (currentPage !== 'index.html') {
            // Não é página de paciente, não atualizar
            return;
        }
        ...
        // Atualizar no HTML usando classes (apenas na página de paciente)
...
async function logout() {
    try {
        ...
        // Limpar localStorage (opcional, mas recomendado)
        localStorage.clear();
...
// Tornar função de logout global
window.logout = logout;

// Adicionar event listener para botão de logout quando o DOM estiver pronto
