...
async function carregarHabitosSupabase() {
  ...
  const { data: habitos, error: errorHabitos } = await supabase
    .from("habit")
    .select("id_habito, nm_habito, tp_habito, id_usuario, nv_01, nv_02, nv_03")
  ...
  habits = (habitos || []).map(h => ({
    ...
    nv_01: h.nv_01 || "Feito!",
    nv_02: h.nv_02 || "Bem feito",
    nv_03: h.nv_03 || "Perfeito",
    levels: { basic: h.nv_01 || "Feito!", intermediate: h.nv_02 || "Bem feito", advanced: h.nv_03 || "Perfeito" }
  }));
    ...
    habitCard.innerHTML = `
      ...
      <div class="habit-levels">
        <div class="level-item">
          <span class="level-badge basic"><i class="fas fa-check"></i> Feito:</span>
          <span class="level-text">${habit.nv_01 || "Feito!"}</span>
        </div>
        <div class="level-item">
          <span class="level-badge intermediate"><i class="fas fa-star"></i> Bem feito:</span>
          <span class="level-text">${habit.nv_02 || "Bem feito"}</span>
        </div>
        <div class="level-item">
          <span class="level-badge advanced"><i class="fas fa-crown"></i> Perfeito:</span>
          <span class="level-text">${habit.nv_03 || "Perfeito"}</span>
        </div>
      </div>
...
async function addNewHabit() {
  ...
  const nv_01 = document.getElementById("level-basic").value.trim();
  const nv_02 = document.getElementById("level-intermediate").value.trim();
  const nv_03 = document.getElementById("level-advanced").value.trim();
  ...
  const { data: habitData, error: errorHabit } = await supabase
    ...
    .insert([{ 
      ...
      id_usuario: USER_ID,
      nv_01: nv_01 || "Feito!",
      nv_02: nv_02 || "Bem feito",
      nv_03: nv_03 || "Perfeito"
    }])
    .select()
    .single();
...
async function editHabit(habitId) {
  ...
  // Preencher campos de níveis
  document.getElementById("level-basic").value = data.nv_01 || "";
  document.getElementById("level-intermediate").value = data.nv_02 || "";
  document.getElementById("level-advanced").value = data.nv_03 || "";
